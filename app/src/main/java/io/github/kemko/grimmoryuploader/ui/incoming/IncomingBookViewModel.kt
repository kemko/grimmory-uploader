...
class IncomingBookViewModel(
    private val container: AppContainer,
) {
    suspend fun persist(
        input: IncomingInput,
        resolver: ContentResolver?,
    ): Result<UploadJobEntity> = runCatching { container.upload.persist(input, resolver) }

    suspend fun prepare(
        jobId: Long,
        beforeSchedule: () -> Unit = {},
    ): Result<IncomingPreparation> =
        runCatching {
