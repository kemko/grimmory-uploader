...
class AuthInterceptorTest {
    @Test
    fun refreshesAndRetriesA401OnlyOnce() =
        runBlocking {
    ...
    @Test
    fun refreshFailureClearsCredentialsAndLeavesRequestUnauthorized() =
        runBlocking {
            val server = MockWebServer()
            server.enqueue(MockResponse().setResponseCode(401))
            server.enqueue(MockResponse().setResponseCode(401))
            ...
            try {
                ...
                val error =
                    org.junit.Assert.assertThrows(io.github.kemko.grimmoryuploader.data.network.ApiException::class.java) {
    ...
    @Test
    fun transientPreRequestRefreshFailureIsRetryable() =
        runBlocking {
            val server = MockWebServer()
    ...
    @Test
    fun transientPostRequestRefreshFailureIsRetryable() =
        runBlocking {
            val server = MockWebServer()
            server.enqueue(MockResponse().setResponseCode(401))
            server.enqueue(MockResponse().setResponseCode(503))
    ...
    @Test
    fun neverSendsBearerTokenOutsideConfiguredServer() =
        runBlocking {
