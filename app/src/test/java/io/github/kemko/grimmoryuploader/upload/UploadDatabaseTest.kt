...
class UploadDatabaseTest {
    @Test
    fun persistsQueriesAndAtomicallyTransitionsJobs() =
        runBlocking {
    ...
    @Test
    fun configuresDraftAndConfirmsCleartextWithGuardedQueries() =
        runBlocking {
            val context = ApplicationProvider.getApplicationContext<Context>()
    ...
    @Test
    fun migratesVersionOneJavaSchemaWithoutLosingQueuedJobs() =
        runBlocking {
            val context = ApplicationProvider.getApplicationContext<Context>()
            ...
            val migrated =
                Room
                    .databaseBuilder(context, UploadDatabase::class.java, name)
    ...
    private fun job(serverUrl: String = "https://one.example") =
        UploadJobEntity(
