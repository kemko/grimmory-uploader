...
class AppViewModelTest {
    ...
    @Test
    fun incomingPreparationPersistsBeforeAuthAndSchedulesAuthenticatedInput() =
        runBlocking {
    ...
    @Test
    fun incomingPreparationRequiresAuthAfterRejectedRefreshAndPropagatesTransientErrors() =
        runBlocking {
            val server = MockWebServer()
            server.start()
            try {
                ...
                val rejected =
                    viewModel
                        .persist(IncomingInput.Url("https://books.test/one.fb2", "one.fb2"), null)
                        .getOrThrow()
                ...
                val transient =
                    viewModel
                        .persist(IncomingInput.Url("https://books.test/two.fb2", "two.fb2"), null)
                        .getOrThrow()
                container.tokenStore.write(TokenPair("expired", "refresh", 0, serverUrl))
    ...
    @Test
    fun concurrentUnauthorizedRequestsRefreshWithoutDispatcherDeadlock() =
        runBlocking {
            val server = MockWebServer()
    ...
    @Test
    fun homeActionsAndConfirmedServerChangeRemoveAllOldServerJobs() =
        runBlocking {
    ...
    @Test
    fun authViewModelCoversModeLoginUserAndResume() =
        runBlocking {
            val server = MockWebServer()
            server.start()
            try {
    ...
    @Test
    fun authViewModelClearsOnlyRejectedTokens() =
        runBlocking {
            val server = MockWebServer()
            server.start()
            try {
